...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        //initialize the user with the given name 
        User user = getUser(name);
        if (user == null) {
            return null;
        }
       
        User mostRecomendedUserToFollow = null;
        ...
        for(int i = 0; i < users.length; i++) {
            ...
            int mutualFollowees = user.countMutual(users[i]);
             if(mutualFollowees > maxMutualFollowees) {
                ...
                mostRecomendedUserToFollow = users[i];
             }
            }
        }

        return mostRecomendedUserToFollow.getName();
    }
    ...
    public String mostPopularUser() {
        ...
        for(int i = 0; i < userCount;i++) {
            int followees = followeeCount(users[i].getName());
            if(followees > followeeCounter) {
                followeeCounter = followees;
